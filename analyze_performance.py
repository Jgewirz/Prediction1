...
def analyze_decisions(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    main_bets = (
        actionable[actionable["is_hedge"] != True]
        if "is_hedge" in actionable.columns
        else actionable
    )
    hedges = (
        actionable[actionable["is_hedge"] == True]
    ...
    if len(actionable) > 0:
        ...
        metrics["high_confidence_bets"] = len(
            actionable[actionable["confidence"] >= 0.7]
        )
        ...
        if "expected_return" in actionable.columns:
            ...
            if len(exp_returns) > 0:
                ...
                metrics["total_expected_value"] = (
                    actionable["bet_amount"] * actionable["expected_return"]
                ).sum()
        ...
        if (
            "research_probability" in actionable.columns
            and "calc_market_prob" in actionable.columns
        ):
            actionable["edge"] = (
                actionable["research_probability"] - actionable["calc_market_prob"]
            )
...
def analyze_by_event(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Group by event
    event_stats = (
        actionable.groupby("event_ticker")
        .agg(
            {
    ...
    for action in ["buy_yes", "buy_no"]:
        ...
        if len(subset) > 0:
            result[action] = {
                ...
                "avg_r_score": (
                    subset["r_score"].mean() if "r_score" in subset.columns else None
                ),
            }
...
def print_summary(metrics: Dict[str, Any]):
    ...
    console.print("\n")
    console.print(
        Panel.fit("[bold]TRADING BOT PERFORMANCE SUMMARY[/bold]", style="blue")
    )
    ...
    if metrics.get("actionable_decisions", 0) > 0:
        ...
        table2.add_row(
            "Total Capital", f"${metrics.get('total_capital_deployed', 0):,.2f}"
        )
        ...
        table3.add_row(
            "High Confidence Bets (≥0.7)", str(metrics.get("high_confidence_bets", 0))
        )
        ...
        if "avg_expected_return" in metrics:
            table3.add_row(
                "Avg Expected Return", f"{metrics.get('avg_expected_return', 0):.1%}"
            )
            table3.add_row(
                "Total Expected Value",
                f"${metrics.get('total_expected_value', 0):,.2f}",
            )

        if "avg_kelly_fraction" in metrics:
            table3.add_row(
                "Avg Kelly Fraction", f"{metrics.get('avg_kelly_fraction', 0):.3f}"
            )
        ...
        if total > 0:
            table4.add_row(
                "BUY YES",
                str(metrics.get("buy_yes_count", 0)),
                f"{metrics.get('buy_yes_count', 0)/total:.1%}",
            )
            table4.add_row(
                "BUY NO",
                str(metrics.get("buy_no_count", 0)),
...
def print_calibration_summary():
    ...
    try:
        ...
        if stats["resolved_predictions"] > 0:
            console.print("\n")
            console.print(
                Panel.fit("[bold]CALIBRATION METRICS[/bold]", style="magenta")
            )
            ...
            if stats["confidence_calibration"] is not None:
                table.add_row(
                    "Calibration Error", f"{stats['confidence_calibration']:.3f}"
                )
            ...
            if stats.get("bucket_accuracy"):
                bucket_table = Table(
                    title="Accuracy by Confidence Bucket", show_header=True
                )
                ...
                for bucket, data in stats["bucket_accuracy"].items():
                    bucket_table.add_row(
                        bucket, str(data["count"]), f"{data['accuracy']:.1%}"
                    )

                console.print(bucket_table)
        else:
            console.print(
                "[yellow]No resolved predictions yet for calibration analysis[/yellow]"
            )
...
async def main_async(args):
    ...
    console.print(
        "[bold blue]Kalshi Deep Trading Bot - Performance Analysis[/bold blue]\n"
    )
    ...
    if args.pnl:
        ...
        if pnl_data:
            ...
            console.print(
                "[yellow]No P&L data available. Run --reconcile first.[/yellow]"
            )
        return
        ...
        if len(event_stats) > 0:
            ...
            for event_ticker, row in event_stats.head(15).iterrows():
                table.add_row(
                    ...
                    (
                        f"{row['avg_r_score']:.3f}"
                        if pd.notna(row["avg_r_score"])
                        else "N/A"
                    ),
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze Kalshi trading bot performance"
    )
    parser.add_argument(
        "--csv-dir", default="betting_decisions", help="Directory containing CSV files"
    )
    parser.add_argument(
        "--db",
        action="store_true",
        help="Load data from SQLite database instead of CSV",
    )
    parser.add_argument(
        "--pnl", action="store_true", help="Show P&L summary from settled bets"
    )
    parser.add_argument(
        "--detailed", action="store_true", help="Show detailed per-event analysis"
    )
