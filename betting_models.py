...
class MarketProbability(BaseModel):
    ...
    research_probability: float = Field(
        ...
        ge=0,
        le=100,
        description="Research predicted probability (0-100 from LLM, converted to 0-1 for storage)",
    )
    reasoning: str = Field(
        ..., description="Brief reasoning for the probability estimate"
    )
...
class ProbabilityExtraction(BaseModel):
    ...
    markets: List[MarketProbability] = Field(
        ..., description="List of market probabilities"
    )
...
class BettingDecision(BaseModel):
    ...
    confidence: float = Field(
        ..., ge=0, le=1, description="Confidence in decision (0-1)"
    )
    ...
    hedge_for: Optional[str] = Field(
        None, description="Ticker of the main bet this hedges"
    )
    hedge_ratio: Optional[float] = Field(
        None, ge=0, le=1, description="Proportion of main bet this hedges"
    )

    # Risk-adjusted metrics (hedge-fund style)
    expected_return: Optional[float] = Field(
        None, description="Expected return on capital E[R] = (p-y)/y"
    )
    r_score: Optional[float] = Field(
        None, description="Risk-adjusted edge: (p-y)/sqrt(p*(1-p)) - the z-score"
    )
    kelly_fraction: Optional[float] = Field(
        None, description="Optimal Kelly fraction for position sizing"
    )
    market_price: Optional[float] = Field(
        None,
        ge=0,
        le=1,
        description="Market price used for calculations (0-1 normalized)",
    )
    research_probability: Optional[float] = Field(
        None,
        ge=0,
        le=1,
        description="Research probability used for calculations (0-1 normalized)",
    )
    ...
    signal_applied: bool = Field(
        False, description="Whether a TrendRadar signal was applied"
    )
    signal_direction: Optional[str] = Field(
        None, description="'aligned', 'conflicting', or 'neutral'"
    )
    signal_topic: Optional[str] = Field(None, description="Trending topic matched")
    signal_sentiment: Optional[str] = Field(
        None, description="'positive', 'negative', or 'neutral'"
    )
    signal_strength: Optional[float] = Field(
        None, ge=0, le=1, description="Signal strength 0.0 to 1.0"
    )
    signal_source_count: Optional[int] = Field(
        None, ge=0, description="Number of news sources"
    )
    confidence_boost: float = Field(
        0.0, description="Confidence adjustment from signal"
    )
    kelly_multiplier: float = Field(
        1.0, description="Position size multiplier from signal"
    )
    override_skip_triggered: bool = Field(
        False, description="Whether skip was overridden by signal"
    )
    signal_reasoning: Optional[str] = Field(
        None, description="Explanation of signal impact"
    )

    # NEW v2: Enhanced signal metrics
    signal_unique_stories: Optional[int] = Field(
        None, description="Unique stories after de-dup"
    )
    signal_unique_outlets: Optional[int] = Field(
        None, description="Unique news outlets"
    )
    signal_tier1_count: Optional[int] = Field(None, description="Tier-1 outlet count")
    signal_relevance: Optional[float] = Field(
        None, description="Semantic relevance to event"
    )
    signal_frequency_ratio: Optional[float] = Field(
        None, description="Current vs baseline frequency"
    )
    probability_adjustment: float = Field(
        0.0, description="Probability shift from signal (single-lever)"
    )

    # Phase 7: Skip reason tracking for safe override
    skip_reason: Optional[str] = Field(
        None, description="Why the bet was skipped (SkipReason enum value)"
    )
    override_blocked: bool = Field(
        False, description="True if signal tried to override but was blocked"
    )
    original_action: Optional[str] = Field(
        None, description="Original action before signal override"
    )
...
class MarketAnalysis(BaseModel):
    ...
    total_recommended_bet: float = Field(
        ..., description="Total amount recommended to bet"
    )
    high_confidence_bets: int = Field(
        ..., description="Number of high confidence bets (>0.7)"
    )
