...
class DatabaseConfig(BaseModel):
    ...
    db_type: str = Field(default="postgres", description="Database type: 'sqlite' or 'postgres'")

    # SQLite settings
    db_path: str = Field(default="trading_bot.db", description="SQLite database file path")
    ...
    pg_password: str = Field(default="", description="PostgreSQL password (set via PGPASSWORD)")
    ...
    save_to_csv: bool = Field(default=True, description="Also save to CSV files (backup)")
    migrate_on_startup: bool = Field(default=True, description="Run schema migrations on startup")
    auto_reconcile: bool = Field(default=True, description="Auto-reconcile outcomes after bot run")
...
class SchedulerConfig(BaseModel):
    ...
    startup_delay_seconds: int = Field(default=10, ge=0, le=60, description="Delay before first run")


class TrendRadarConfig(BaseModel):
    """TrendRadar news intelligence integration configuration."""
    enabled: bool = Field(default=True, description="Enable TrendRadar integration for news signals")
    base_url: str = Field(default="http://localhost:3333", description="TrendRadar MCP server URL")
    timeout: float = Field(default=10.0, description="Request timeout in seconds (reduced from 30s for faster failures)")

    # Retry settings
    max_retries: int = Field(default=2, ge=0, le=5, description="Max retry attempts for failed requests")
    retry_backoff_base: float = Field(default=1.0, ge=0.5, le=5.0, description="Base backoff seconds for exponential retry")

    # Circuit breaker settings
    circuit_failure_threshold: int = Field(default=3, ge=1, le=10, description="Consecutive failures before opening circuit")
    circuit_reset_seconds: int = Field(default=300, ge=60, le=900, description="Seconds before circuit breaker resets")

    # Signal influence settings
    max_confidence_boost: float = Field(default=0.30, ge=0.0, le=0.5, description="Maximum confidence boost from aligned signals")
    strong_signal_threshold: float = Field(default=0.7, ge=0.0, le=1.0, description="Threshold for strong signal classification")
    min_source_count: int = Field(default=2, ge=1, description="Minimum news sources for valid signal")

    # Position sizing adjustments
    aligned_signal_kelly_multiplier: float = Field(default=1.25, ge=1.0, le=2.0, description="Kelly multiplier for strong aligned signals")

    # Skip override settings
    enable_skip_override: bool = Field(default=True, description="Allow strong signals to override skip decisions")
    skip_override_min_strength: float = Field(default=0.8, ge=0.5, le=1.0, description="Minimum signal strength for skip override")
    skip_override_min_sources: int = Field(default=5, ge=1, description="Minimum sources for skip override")

    # Ablation mode for A/B testing ROI lift
    ablation_rate: float = Field(default=0.0, ge=0.0, le=1.0, description="Fraction of events to ignore signal influence for A/B testing (0=disabled)")

    # Cache settings
    cache_ttl_seconds: float = Field(default=300.0, ge=60, le=1800, description="Signal cache TTL in seconds")


class StopLossConfig(BaseModel):
    """Stop-loss and take-profit configuration for automated position exits."""
    enabled: bool = Field(default=True, description="Enable stop-loss/take-profit monitoring")

    # Default thresholds (can be overridden per-position)
    default_stop_loss_pct: float = Field(default=0.15, ge=0.01, le=0.50, description="Default stop-loss percentage (0.15 = exit if down 15%)")
    default_take_profit_pct: float = Field(default=0.30, ge=0.05, le=1.0, description="Default take-profit percentage (0.30 = exit if up 30%)")

    # Trailing stop (optional)
    trailing_stop_enabled: bool = Field(default=False, description="Enable trailing stop-loss")
    trailing_stop_pct: float = Field(default=0.10, ge=0.01, le=0.30, description="Trail stop this percentage below high water mark")

    # Monitoring settings
    monitor_interval_seconds: int = Field(default=30, ge=5, le=300, description="Seconds between position checks")
    price_staleness_seconds: int = Field(default=60, ge=10, le=300, description="Re-fetch price if older than this")

    # Execution settings
    use_market_orders: bool = Field(default=False, description="Use market orders for exits (True) or limit at bid (False)")
    slippage_tolerance_pct: float = Field(default=0.02, ge=0.0, le=0.10, description="Maximum slippage tolerance for limit orders")
    retry_failed_exits: bool = Field(default=True, description="Retry failed exit orders")
    max_exit_retries: int = Field(default=3, ge=1, le=10, description="Maximum retry attempts for exit orders")

    # Risk controls
    max_simultaneous_exits: int = Field(default=5, ge=1, le=20, description="Maximum positions to exit simultaneously")
    min_position_age_seconds: int = Field(default=300, ge=0, le=3600, description="Don't exit positions younger than this (anti-churn)")

    # Alerting
    alert_on_trigger: bool = Field(default=True, description="Send alert when SL/TP triggers")
...
class BotConfig(BaseSettings):
    ...
    # Hedging settings
    enable_hedging: bool = Field(default=False, description="Disabled - hedging reduces ROI by ~25% on winning bets")
    ...
    # =========================================================================
    max_daily_loss: float = Field(default=100.0, ge=0, description="Maximum daily loss before kill switch triggers ($)")
    max_daily_loss_pct: float = Field(default=0.10, ge=0, le=0.5, description="Maximum daily loss as fraction of bankroll (10%)")
    enable_kill_switch: bool = Field(default=True, description="Enable automatic trading halt on daily loss limit")
    max_total_exposure: float = Field(default=500.0, ge=0, description="Maximum total notional exposure across all positions")

    # Live trading safety
    require_live_confirmation: bool = Field(default=True, description="Require explicit confirmation for live trading")
    ...
    def __init__(self, **data):
        ...
        database_config = DatabaseConfig(
            ...
            pg_password=os.getenv("PGPASSWORD", ""),  # Required: set via PGPASSWORD env var
            pg_port=int(os.getenv("PGPORT", "5432")),
            pg_ssl=os.getenv("PGSSLMODE", "require"),
            enable_db=_clean_env_value(os.getenv("ENABLE_DB", "true")).lower() == "true",
            save_to_csv=_clean_env_value(os.getenv("SAVE_TO_CSV", "true")).lower() == "true",
            migrate_on_startup=_clean_env_value(os.getenv("DB_MIGRATE_ON_STARTUP", "true")).lower() == "true",
            auto_reconcile=_clean_env_value(os.getenv("AUTO_RECONCILE", "true")).lower() == "true"
        )

        trendradar_config = TrendRadarConfig(
            enabled=_clean_env_value(os.getenv("TRENDRADAR_ENABLED", "true")).lower() == "true",
            ...
            retry_backoff_base=float(_clean_env_value(os.getenv("TRENDRADAR_RETRY_BACKOFF", "1.0"))),
            # Circuit breaker settings
            circuit_failure_threshold=int(_clean_env_value(os.getenv("TRENDRADAR_CIRCUIT_THRESHOLD", "3"))),
            circuit_reset_seconds=int(_clean_env_value(os.getenv("TRENDRADAR_CIRCUIT_RESET", "300"))),
            # Signal influence settings
            max_confidence_boost=float(_clean_env_value(os.getenv("TRENDRADAR_MAX_BOOST", "0.30"))),
            strong_signal_threshold=float(_clean_env_value(os.getenv("TRENDRADAR_STRONG_THRESHOLD", "0.7"))),
            min_source_count=int(_clean_env_value(os.getenv("TRENDRADAR_MIN_SOURCES", "2"))),
            aligned_signal_kelly_multiplier=float(_clean_env_value(os.getenv("TRENDRADAR_KELLY_MULTIPLIER", "1.25"))),
            enable_skip_override=_clean_env_value(os.getenv("TRENDRADAR_SKIP_OVERRIDE", "true")).lower() == "true",
            skip_override_min_strength=float(_clean_env_value(os.getenv("TRENDRADAR_SKIP_OVERRIDE_STRENGTH", "0.8"))),
            skip_override_min_sources=int(_clean_env_value(os.getenv("TRENDRADAR_SKIP_OVERRIDE_SOURCES", "5"))),
            # Ablation testing and caching
            ablation_rate=float(_clean_env_value(os.getenv("TRENDRADAR_ABLATION_RATE", "0.0"))),
            cache_ttl_seconds=float(_clean_env_value(os.getenv("TRENDRADAR_CACHE_TTL", "300.0")))
        )
        ...
        scheduler_config = SchedulerConfig(
            ...
            startup_delay_seconds=int(_clean_env_value(os.getenv("SCHEDULER_STARTUP_DELAY", "10")))
        )

        stop_loss_config = StopLossConfig(
            enabled=_clean_env_value(os.getenv("SL_TP_ENABLED", "true")).lower() == "true",
            default_stop_loss_pct=float(_clean_env_value(os.getenv("DEFAULT_STOP_LOSS_PCT", "0.15"))),
            default_take_profit_pct=float(_clean_env_value(os.getenv("DEFAULT_TAKE_PROFIT_PCT", "0.30"))),
            trailing_stop_enabled=_clean_env_value(os.getenv("TRAILING_STOP_ENABLED", "false")).lower() == "true",
            trailing_stop_pct=float(_clean_env_value(os.getenv("TRAILING_STOP_PCT", "0.10"))),
            monitor_interval_seconds=int(_clean_env_value(os.getenv("MONITOR_INTERVAL_SECONDS", "30"))),
            price_staleness_seconds=int(_clean_env_value(os.getenv("PRICE_STALENESS_SECONDS", "60"))),
            use_market_orders=_clean_env_value(os.getenv("USE_MARKET_ORDERS", "false")).lower() == "true",
            slippage_tolerance_pct=float(_clean_env_value(os.getenv("SLIPPAGE_TOLERANCE_PCT", "0.02"))),
            retry_failed_exits=_clean_env_value(os.getenv("RETRY_FAILED_EXITS", "true")).lower() == "true",
            max_exit_retries=int(_clean_env_value(os.getenv("MAX_EXIT_RETRIES", "3"))),
            max_simultaneous_exits=int(_clean_env_value(os.getenv("MAX_SIMULTANEOUS_EXITS", "5"))),
            min_position_age_seconds=int(_clean_env_value(os.getenv("MIN_POSITION_AGE_SECONDS", "300"))),
            alert_on_trigger=_clean_env_value(os.getenv("ALERT_ON_TRIGGER", "true")).lower() == "true",
        ...
        data.update({
            ...
            "max_daily_loss": float(_clean_env_value(os.getenv("MAX_DAILY_LOSS", "100.0"))),
            "max_daily_loss_pct": float(_clean_env_value(os.getenv("MAX_DAILY_LOSS_PCT", "0.10"))),
            "enable_kill_switch": _clean_env_value(os.getenv("ENABLE_KILL_SWITCH", "true")).lower() == "true",
            "max_total_exposure": float(_clean_env_value(os.getenv("MAX_TOTAL_EXPOSURE", "500.0"))),
            "require_live_confirmation": _clean_env_value(os.getenv("REQUIRE_LIVE_CONFIRMATION", "true")).lower() == "true"
        })
