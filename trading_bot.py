...
from dashboard.broadcaster import (broadcast_alert, broadcast_decision,
                                   broadcast_kpi_update, broadcast_status,
                                   broadcast_workflow_step)
from db import Database, PostgresDatabase, get_database, get_postgres_database
...
from trendradar_client import (SignalConfig, TrendingSignal, TrendRadarClient,
...
class SimpleTradingBot:
    ...
    def __init__(self, live_trading: bool = False, max_close_ts: Optional[int] = None):
        ...
        self.event_signals: Dict[str, List[TrendingSignal]] = (
            {}
        )  # Cache signals per event
        ...
        self._calibration_enabled = (
            os.environ.get("CALIBRATION_ENABLED", "true").lower() == "true"
        )
        self._calibration_min_samples = int(
            os.environ.get("CALIBRATION_MIN_SAMPLES", "20")
        )
    ...
    async def initialize(self):
        ...
        if self.config.database.enable_db:
            try:
                ...
                if db_type == "postgres":
                    ...
                    self.console.print(
                        "[green][OK] PostgreSQL database connected (Neon)[/green]"
                    )
                else:
                    # Use SQLite
                ...
                # Record run start
                await self.db.start_run(
                    self.run_id,
                    {
                        "mode": "dry_run" if self.config.dry_run else "live",
                        "environment": (
                            "demo" if self.config.kalshi.use_demo else "production"
                        ),
        ...
        if self.config.trendradar.enabled:
            ...
            if await self.trendradar_client.health_check():
                ...
                self.console.print(
                    "[green][OK] TrendRadar connected (Western Financial News)[/green]"
                )
            else:
                logger.warning(
                ...
                self.console.print(
                    "[yellow][!] TrendRadar not reachable - continuing without news signals[/yellow]"
                )
                self.trendradar_client.enabled = False
        else:
            logger.info(
                "TrendRadar integration disabled via config (TRENDRADAR_ENABLED=false)"
            )
        ...
        if self._calibration_enabled:
            ...
            self.console.print(
                "[blue]Calibration disabled (CALIBRATION_ENABLED=false)[/blue]"
            )
        ...
        self.console.print(
            f"[blue]Research batch size: {self.config.research_batch_size}[/blue]"
        )
        self.console.print(
        ...
        self.console.print(
            f"[blue]Max bet amount: ${self.config.max_bet_amount}[/blue]"
        )
    ...
    async def check_kill_switch(self) -> bool:
        ...
        try:
            ...
            if row:
                raw_pnl = (
                    row.get("daily_pnl", 0.0)
                    if isinstance(row, dict)
                    else (row[0] if row[0] is not None else 0.0)
                )
                ...
                pct_loss = (
                    abs(daily_pnl) / self.config.bankroll
                ...
                if daily_pnl != 0:
                    logger.info(
                        f"Daily P&L: ${daily_pnl:.2f} (limit: ${self.config.max_daily_loss:.2f})"
                    )
    ...
    def calculate_risk_adjusted_metrics(
        ...
        try:
            ...
            if action == "buy_yes":
                ...
                p = (
                    1 - research_prob
                )  # Our probability of NO (1 - research_prob_of_yes)
    ...
    def calculate_dynamic_kelly(
        ...
        quality_multiplier = (
            r_score_multiplier * confidence_multiplier * signal_multiplier
        )
    ...
    def calculate_quality_adjusted_position_size(
        self, kelly_fraction: float, r_score: float, confidence: float
    ) -> float:
        """
        ...
        adjusted_kelly = self.calculate_dynamic_kelly(
            kelly_fraction, r_score, confidence
        )
    ...
    def apply_portfolio_selection(
        ...
        if self.config.portfolio_selection_method == "top_r_scores":
            ...
            analysis.decisions = (
                selected_decisions + skip_decisions + rejected_decisions
            )
    ...
    async def get_markets_for_events(
        ...
        for event in events:
            ...
            if markets:
                ...
                if total_markets > len(markets):
                    self.console.print(
                        f"[green][OK] Using top {len(markets)} markets for {event_ticker} (from {total_markets} total)[/green]"
                    )
                else:
                    self.console.print(
                        f"[green][OK] Using {len(markets)} markets for {event_ticker}[/green]"
                    )
            else:
                self.console.print(
        ...
        self.console.print(
            f"[green][OK] Processing {total_markets} total markets across {len(event_markets)} events[/green]"
        )
    ...
    async def filter_markets_by_positions(
            ...
            for event_ticker, data in event_markets.items():
                ...
                for market in markets:
                    ...
                    try:
                        ...
                        if has_position:
                            self.console.print(
                                f"[yellow][!] Skipping {ticker}: Has existing position[/yellow]"
                            )
                            skipped_markets += 1
                        else:
                            # Keep this market
                ...
                if filtered_markets:
                    ...
                    if len(filtered_markets) < len(markets):
                        self.console.print(
                            f"[blue]• Event {event_ticker}: Kept {len(filtered_markets)}/{len(markets)} markets[/blue]"
                        )
                    else:
                        self.console.print(
                            f"[green][OK] Event {event_ticker}: All {len(markets)} markets available[/green]"
                        )
                else:
                    self.console.print(
                        f"[yellow][!] Event {event_ticker}: All markets have positions, skipping[/yellow]"
                    )
        ...
        self.console.print(
            f"[blue]• Total markets checked: {total_markets_before}[/blue]"
        )
        self.console.print(
            f"[blue]• Markets with positions (skipped): {skipped_markets}[/blue]"
        )
        self.console.print(
            f"[blue]• Markets remaining for research: {total_markets_after}[/blue]"
        )
        self.console.print(
            f"[blue]• Events with available markets: {events_with_some_markets}[/blue]"
        )

        if events_with_some_markets == 0:
            self.console.print(
                "[yellow][!] No markets remaining after position filtering[/yellow]"
            )
    ...
    async def fetch_trending_signals(
        ...
        with Progress(
            ...
            task = progress.add_task(
                "Fetching news signals...", total=len(event_markets)
            )
            ...
            for event_ticker, data in event_markets.items():
                ...
                try:
                    signals = await self.trendradar_client.get_signals_for_event(
                        event_title=event_title, event_category=event_category
                    )
        ...
        self.console.print(
            f"[green][OK] Fetched {total_signals} signals for {len(signals_by_event)} events[/green]"
        )
    ...
    async def research_events(
        ...
        signals_by_event: Dict[str, List[TrendingSignal]] = None,
    ) -> Dict[str, str]:
                ...
                for event_ticker, data in batch:
                    ...
                    if event and markets:
                        ...
                        if event_ticker in signals_by_event:
                            trending_context = format_signals_for_research(
                                signals_by_event[event_ticker]
                            )

                        coro = self.research_client.research_event(
                            event, markets, trending_context
                        )
                        # Apply per-event timeout to avoid hanging the whole batch
                        tasks.append(
                            asyncio.wait_for(
                                coro, timeout=self.config.research_timeout_seconds
                            )
                        )
                    else:
                ...
                try:
                    ...
                    for (event_ticker, data), result in zip(batch, results):
                        if not isinstance(result, Exception) and result:
                            ...
                            progress.update(task, advance=1)
                            self.console.print(
                                f"[green][OK] Researched {event_ticker}[/green]"
                            )
                        else:
                            err = result
                            ...
                            self.console.print(
                                f"[red][X] Failed to research {event_ticker}: {err}[/red]"
                            )
                            progress.update(task, advance=1)
        ...
        self.console.print(
            f"[green][OK] Completed research on {len(research_results)} events[/green]"
        )
        ...
        with Progress(
            ...
            for result in results:
                ...
                if extraction is not None:
                    ...
                    self.console.print(
                        f"[green][OK] Extracted probabilities for {event_ticker}[/green]"
                    )

                    # Display extracted probabilities
                    self.console.print(
                        f"[blue]Extracted probabilities for {event_ticker}:[/blue]"
                    )
                    ...
                    self.console.print(
                        f"[red][X] Failed to extract probabilities for {event_ticker}[/red]"
                    )

                progress.update(task, advance=1)

        self.console.print(
            f"[green][OK] Extracted probabilities for {len(probability_extractions)} events[/green]"
        )
    ...
    async def get_market_odds(
                ...
                try:
                    ...
                    for ticker, result in zip(batch, results):
                        if not isinstance(result, Exception) and result:
                            ...
                            self.console.print(
                                f"[red][X] Failed to get odds for {ticker}[/red]"
                            )
                            progress.update(task, advance=1)
        ...
        self.console.print(
            f"[green][OK] Fetched odds for {len(market_odds)} markets[/green]"
        )
        return market_odds
    ...
    async def get_betting_decisions(
        ...
        with Progress(
            ...
            for result in results:
                ...
                if event_analysis is not None:
                    ...
                    self.console.print(
                        f"[green][OK] Generated {len(event_analysis.decisions)} decisions for {event_ticker}[/green]"
                    )
                else:
                    self.console.print(
                        f"[red][X] Failed to generate decisions for {event_ticker}[/red]"
                    )
        ...
        if actionable_decisions:
            ...
            for decision in actionable_decisions:
                ...
                event_name = (
                    decision.event_name if decision.event_name else "Unknown Event"
                )
                market_name = (
                    decision.market_name if decision.market_name else decision.ticker
                )
    ...
    def _display_event_decisions(
        ...
        mutually_exclusive_adjustments = [
            ...
            for decision in event_analysis.decisions
            if decision.action != "skip"
            and "Mutually exclusive hedge" in decision.reasoning
        ]
        ...
        if not actionable_decisions:
            self.console.print(
                f"[yellow]No actionable decisions for {event_ticker}[/yellow]"
            )
            return

        # Create event-specific table
        event_name = (
            actionable_decisions[0].event_name
            if actionable_decisions
            else "Unknown Event"
        )
    ...
    async def _get_event_betting_decisions(
        ...
        try:
            ...
            if is_mutually_exclusive:
                analysis = self._enforce_mutually_exclusive_constraint(
    ...
    def _add_human_readable_names(
        ...
        market_names = {
            market.get("ticker", ""): market.get("title", "") for market in markets
        }
    ...
    def _apply_alpha_threshold_validation(
        ...
        for decision in analysis.decisions:
            ...
            if decision.action == "skip":
                ...
                if (
                    research_prob is not None
                    and market_odds_data is not None
                    and market_odds_data > 0
                ):
                    risk_metrics = self.calculate_risk_adjusted_metrics(
                        research_prob,
                        market_odds_data,
                        "buy_yes",  # Use buy_yes for skip metric calc
                    )
            ...
            if event_ticker in self.event_signals:
                for signal in self.event_signals[event_ticker]:
                    ...
                    if signal.matches_event(decision.market_name or decision.ticker):
                        ...
                        if abs(influence["confidence_boost"]) > abs(
                            signal_influence["confidence_boost"]
                        ):
                            signal_influence = influence
                            best_signal = (
                                signal  # Track the best signal for persistence
                            )

            # Apply confidence boost from signals
            original_confidence = decision.confidence
            boosted_confidence = (
                decision.confidence + signal_influence["confidence_boost"]
            )
            ...
            if best_signal is not None:
                ...
                decision.confidence_boost = signal_influence.get(
                    "confidence_boost", 0.0
                )
                decision.kelly_multiplier = signal_influence.get(
                    "kelly_multiplier", 1.0
                )
                decision.override_skip_triggered = signal_influence.get(
                    "should_override_skip", False
                )
            ...
            if risk_metrics["r_score"] >= self.config.z_threshold:
                ...
                should_accept = True
                logger.info(
                    f"Signal override for {decision.ticker}: {signal_influence['reasoning']}"
                )
            else:
            ...
            if should_accept:
                ...
                skip_decision = BettingDecision(
                    ...
                    override_skip_triggered=getattr(
                        decision, "override_skip_triggered", False
                    ),
        ...
        analysis.high_confidence_bets = len(
            [
                ...
                if d.action != "skip" and d.confidence > 0.7
            ]
        )
        ...
        for decision in actionable_decisions:
            if self.config.dry_run:
                ...
                if result.get("success"):
                    self.console.print(
                        f"[green][OK] Placed {decision.action} bet of ${decision.amount} on {decision.ticker}[/green]"
                    )
                else:
                    self.console.print(
                        f"[red][X] Failed to place bet on {decision.ticker}: {result.get('error', 'Unknown error')}[/red]"
                    )
    ...
    def save_betting_decisions_to_csv(
        ...
        for decision in analysis.decisions:
                ...
                for market_prob in extraction.markets:
                    if market_prob.ticker == decision.ticker:
                        ...
                        research_probability = (
                            market_prob.research_probability / 100.0
                            if market_prob.research_probability is not None
                            else None
                        )
            ...
            if decision.ticker in market_odds:
                ...
                market_yes_mid = (
                    (yes_bid + yes_ask) / 2 if yes_bid > 0 and yes_ask > 0 else None
                )
                market_no_mid = (
                    (no_bid + no_ask) / 2 if no_bid > 0 and no_ask > 0 else None
                )
                ...
                market_enriched.update(
                    {
                        ...
                        "market_latest_expiration_time": m.get(
        ...
        if csv_data:
            ...
            self.console.print(
                f"[bold green][OK][/bold green] Betting decisions saved to: [blue]{filepath}[/blue]"
            )
        else:
    ...
    async def save_decisions_to_db(
        ...
        logger.info(
            f"Starting save_decisions_to_db with {len(analysis.decisions)} decisions"
        )
        ...
        for decision in analysis.decisions:
            ...
            decision_record = {
                ...
                "research_summary": research_summary,
                "raw_research": (
                    raw_research[:10000] if raw_research else ""
                ),  # Truncate for DB
                "market_yes_price": market_data.get("yes_ask"),
                "market_no_price": market_data.get("no_ask"),
                "market_yes_mid": (
                    (market_data.get("yes_bid", 0) + market_data.get("yes_ask", 0)) / 2
                    if market_data.get("yes_bid") and market_data.get("yes_ask")
                    else None
                ),
                "market_no_mid": (
                    (market_data.get("no_bid", 0) + market_data.get("no_ask", 0)) / 2
                    if market_data.get("no_bid") and market_data.get("no_ask")
                    else None
                ),
                "expected_return": decision.expected_return,
                "r_score": decision.r_score,
                ...
                "market_close_time": self._parse_datetime(
            ...
            for evt_ticker, data in event_markets.items():
                ...
                for market in data.get("markets", []):
                    if market.get("ticker") == decision.ticker:
                        decision_record["is_unique_event"] = event.get(
                            "is_unique_event", False
                        )
                        decision_record["is_new_market"] = market.get(
                            "is_new_market", False
                        )
                        decision_record["early_entry_score"] = market.get(
        ...
        if decisions_to_save:
            try:
                ...
                self.console.print(
                    f"[green][OK] Saved {count} decisions to SQLite database[/green]"
                )
                return count
            except Exception as e:
    ...
    async def run(self):
        ...
        try:
            ...
            if await self.check_kill_switch():
                self.console.print(
                    "[bold red]Kill switch active - daily loss limit exceeded. Trading halted.[/bold red]"
                )
                if self.db:
            ...
            early_entry_enabled = (
                self.config.early_entry.enabled
                if hasattr(self.config, "early_entry")
                else False
            )
            ...
            if not events:
                await broadcast_workflow_step(
                    1, "Fetching events", "failed", "No events found"
                )
            ...
            await broadcast_workflow_step(
                ...
                "completed",
                details={
                    "events_count": len(event_markets),
                    "markets_count": total_markets,
                },
            )
            if not event_markets:
                await broadcast_workflow_step(
                    2, "Processing markets", "failed", "No markets found"
                )
            ...
            markets_before = sum(
                len(data["markets"]) for data in event_markets.values()
            )
            ...
            await broadcast_workflow_step(
                ...
                "completed",
                details={
                    "filtered_count": markets_before - markets_after,
                    "remaining_count": markets_after,
                },
            )
            if not event_markets:
                self.console.print(
            ...
            if len(event_markets) > self.config.max_events_to_analyze:
                ...
                event_markets = {
                    event_ticker: data for event_ticker, data, _ in top_events
                }
            ...
            await broadcast_workflow_step(
                ...
                details={
                    ...
                    "events_with_signals": len(signals_by_event),
                },
            )
            ...
            research_results = await self.research_events(
                event_markets, signals_by_event
            )
            await broadcast_workflow_step(
                5,
                "Researching events",
            ...
            if not research_results:
                await broadcast_workflow_step(
                    5, "Researching events", "failed", "No research results"
                )
            ...
            if not market_odds:
                await broadcast_workflow_step(
                    7, "Fetching odds", "failed", "No market odds found"
                )
            ...
            actionable_count = len(
                [d for d in analysis.decisions if d.action != "skip"]
            )
                    ...
                    await broadcast_decision(
                        {
                            ...
                            "market_price": decision.market_price,
                            "reasoning": (
                                decision.reasoning[:200] if decision.reasoning else ""
                            ),
                            "run_mode": (
                                "live" if not self.config.dry_run else "dry_run"
                            ),
                            "signal_applied": getattr(
                                decision, "signal_applied", False
                            ),
                            "signal_direction": getattr(
                                decision, "signal_direction", None
                            ),
            ...
            total_wagered = sum(
                d.amount for d in analysis.decisions if d.action != "skip"
            )
            await broadcast_workflow_step(
                10,
                "Placing bets",
...
async def run_migration():
    ...
    from migrations import migrate_calibration_json, migrate_csv_files
    ...
    if not config.database.enable_db:
        console.print(
            "[red]Database is disabled. Enable it in config to run migration.[/red]"
        )
        return
    ...
    try:
        ...
        json_summary = await migrate_calibration_json(
            db, "calibration_data.json", backup=True
        )
...
async def show_statistics():
    ...
    if not config.database.enable_db:
        console.print(
            "[red]Database is disabled. Enable it in config to use statistics.[/red]"
        )
        return
...
Configuration:
  ...
  Trading modes:
    ...
    parser.add_argument(
        "--version", action="version", version="Kalshi Trading Bot 1.0.0"
    )
