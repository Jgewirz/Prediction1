...
class CalibrationTracker:
    ...
    def _load(self):
        ...
        if self.storage_path.exists():
            try:
                ...
                logger.info(
                    f"Loaded {len(self.data)} calibration records from {self.storage_path}"
                )
    ...
    def record_prediction(
        ...
        self._save()
        logger.debug(
            f"Recorded prediction for {ticker}: prob={predicted_prob:.1f}%, conf={confidence:.2f}"
        )

        return prediction_id

    def record_outcome(
        self, ticker: str, outcome: bool, payout: Optional[float] = None
    ) -> bool:
        """
        ...
        for entry in reversed(self.data):
            if entry["ticker"] == ticker and entry["outcome"] is None:
                ...
                self._save()
                logger.info(
                    f"Recorded outcome for {ticker}: {'YES' if outcome else 'NO'}"
                )
                return True
    ...
    def calculate_brier_score(
        ...
        brier = sum(
            ((e["predicted_prob"] / 100) - e["outcome"]) ** 2 for e in resolved
        ) / len(resolved)
    ...
    def calculate_accuracy(
        ...
        correct = sum(
            1 for e in resolved if (e["predicted_prob"] > 50) == (e["outcome"] == 1.0)
        )
    ...
    def calculate_calibration_stats(self) -> Dict[str, Any]:
        ...
        avg_r_score = statistics.mean(
            e["r_score"] for e in resolved if e["r_score"] is not None
        )
    ...
    def get_bias_analysis(self) -> Dict[str, Any]:
        ...
        if len(resolved) < 10:
            return {
                "message": "Need at least 10 resolved predictions for bias analysis"
            }
        ...
        return {
            ...
            "buy_yes_accuracy": (
                (sum(1 for e in buy_yes if e["outcome"] == 1.0) / len(buy_yes))
                if buy_yes
                else None
            ),
            "buy_no_accuracy": (
                (sum(1 for e in buy_no if e["outcome"] == 0.0) / len(buy_no))
                if buy_no
                else None
            ),
...
class CalibrationCurve:
    ...
    def __init__(
        self, min_samples: int = 20, storage_path: str = "calibration_curve.json"
    ):
        """
                ...
                with open(self.storage_path, "r", encoding="utf-8") as f:
                    ...
                    if "x_points" in data and "y_points" in data:
                        try:
                            ...
                            self._fitted = True
                            logger.info(
                                f"Loaded calibration curve from {self.storage_path} "
                                f"(fitted on {self._last_fit_count} samples)"
                            )
                        except ImportError:
                            logger.warning(
                                "scikit-learn not available - calibration disabled"
                            )
            except Exception as e:
    ...
    def fit(self, predicted_probs: List[float], outcomes: List[float]) -> bool:
        ...
        if len(predicted_probs) < self.min_samples:
            logger.info(
                f"Not enough samples to fit calibration curve "
        ...
        try:
            ...
            logger.error(
                "scikit-learn required for calibration. Install with: pip install scikit-learn"
            )
            return False
        except Exception as e:
...
def calculate_expected_calibration_error(
    predicted_probs: List[float], outcomes: List[float], n_bins: int = 10
) -> float:
    """
...
def get_calibration_curve(
    min_samples: int = 20, storage_path: str = "calibration_curve.json"
) -> CalibrationCurve:
