...
class ReconciliationEngine:
            ...
            for decision in pending:
                ...
                if ticker in settled_lookup:
                    ...
                    if outcome:
                        ...
                        await self._update_calibration_for_decision(decision, outcome, payout)

                        settled_count += 1
                        logger.info(f"Settled {ticker}: {outcome}, P&L: ${profit_loss:.2f}")
                else:
                    still_pending += 1
    ...
    def calculate_pnl(self, action: str, amount: float, outcome: str) -> tuple[float, float]:
        """
    ...
    async def _update_calibration_for_decision(
        ...
        try:
            ...
            logger.debug(f"Could not update calibration for {decision.get('market_ticker')}: {e}")

    async def aggregate_daily_performance(self, target_date: Optional[date] = None) -> None:
        """
        ...
        try:
            ...
            for d in dates_to_process:
                ...
                if row and row['total_bets'] > 0:
                    roi = (row['net_profit_loss'] / row['total_wagered'] * 100) if row['total_wagered'] else 0
                    win_rate = row['winning_bets'] / row['total_bets'] if row['total_bets'] else 0
