...
class PostgresDatabase:
        ...
        async with self._pool.acquire() as conn:
            ...
            for d in decisions:
                ...
                try:
                    ...
                    logger.error(f"Error inserting decision {d.get('decision_id', 'unknown')}: {e}")
    ...
    async def update_decision_outcome(
        ...
        query = """
            ...
            WHERE decision_id = $5
        """
        await self.execute(query, outcome, payout, profit_loss, datetime.utcnow(), decision_id)
    ...
    async def update_calibration_outcome(
        ...
        """
        await self.execute(query, outcome, actual_payout, datetime.utcnow(), prediction_id)
    ...
    async def cache_market(self, market: Dict[str, Any], event_ticker: str) -> None:
        ...
        await self.execute(
            ...
            datetime.utcnow().replace(minute=datetime.utcnow().minute + 15)  # 15 min cache
        )
